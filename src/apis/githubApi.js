...
export const createNewRepo = async (projectId) => {
  try {
    const res = await axiosInstanceNoHeader.post(
      `/github/organizaions/repository`,
    );
...
// 7. 내 깃허브 조직 목록 조회
export const fetchMyOrgList = async () => {
  try {
    const res = await axiosInstanceNoHeader.get(`/github/organizations/my`);
    return res.data.result;
  } catch (error) {
    console.error("조회 실패:", error.message);
  }
